...
const BookCard = ({ book }: BookCardProps) => (
  ...
  >
    <Link to={`/book/${book.id}`} className="book-card block group">
      {/* Cover */}
      <div className={`aspect-[2/3] bg-gradient-to-b ${book.coverColor} relative overflow-hidden`}>
        <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center">
          <div className={`w-12 h-0.5 ${book.coverAccent} mb-4 opacity-60`} />
          ...
          <div className={`w-8 h-0.5 ${book.coverAccent} mt-4 opacity-40`} />
        </div>
        {/* Hover overlay */}
